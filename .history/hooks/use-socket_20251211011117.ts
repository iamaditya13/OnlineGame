...
export function useSocket(userId: string | undefined) {
  ...
  useEffect(() => {
    ...
    const handleAiRummy = () => {
       ...
       // This is a very simplified AI that just keeps the game moving
       makeMove({ action: "draw", from: "deck" })
       setTimeout(() => {
         // Discard a random card (simplified, assuming we have cards)
         // In a real implementation we'd need to know which card to discard
         // For now, we'll just skip the discard to avoid errors or need complex logic
         // Or we could try to discard the last drawn card if we knew it
       }, 500)
    }
